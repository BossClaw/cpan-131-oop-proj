...
public class Print {
    ...
    public static void gameList(List<Game> games, String title, boolean showCount, String color) {
        ...
        for (Game game : games) {
            int titleLength = "Title: ".length() + game.getTitle().length();
        ...
        if (games.size() == 0) {
            ...
            // Has Results
            // create format
            // the last %s is for the whole 'extra' string
            String format = "%-3s ID: %-4s %-" + maxTitleLength + "s  %s%n";

            // Formate info for each game
            for (Game game : games) {
                String titleStr = "Title: " + game.getTitle();
                String extraString = String.format(
                        "Price: $%.2f  isOwned: %s  Tags: %s",
                        game.getPrice(),
                        game.getIsOwned() ? "T" : "F",
                        String.join(", ", game.getTags()));

                // (args '-', 'id', 'title', 'everything else...')
